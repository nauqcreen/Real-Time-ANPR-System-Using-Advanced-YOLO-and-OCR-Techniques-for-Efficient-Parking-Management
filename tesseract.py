...
def check_out(plate_number):
    ...
    fee = round(duration_hours * 120000, 2)  # Fee per hour = 15000 VND
...
# Function to check if plate has exactly 8 alphanumeric characters
...
def handle_input():
                ...
                if frames_processed % frame_skip == 0:
                    ...
                    if plates:
                        for plate in plates:
                            ...
                            plate_text = ocr_tesseract(processed_plate)
                            st.write(f"Detected License Plate: {plate_text}")

                            # If a plate with exactly 7 characters is detected, save it and stop processing
                            if len(plate_text) == 7:
    ...
    # Handle webcam input
        ...
        if cap.isOpened():
                ...
                if frames_processed % frame_skip == 0:
                    ...
                    if plates:
                        for plate in plates:
                            ...
                            # Check if plate is valid (8 alphanumeric characters)
            ...
            # Release the webcam resource after processing
            cap.release()
    ...
    if image is not None:
        ...
        if len(plates) > 0:
            for plate in plates:
                ...
                # Perform OCR to extract characters
                plate_text = ocr_tesseract(processed_img)
